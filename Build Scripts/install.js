  ...
  return done => {
    ...
    var child = exec(
      ...
      (error, stdout, stderr) => {
        ...
        var powerShellModules = require("glob").sync(
        ...
        if (powerShellModules.length > 0) {
          ...
          var task = fs.existsSync(taskFilePath)
          ...
          if (task.execution.PowerShell3) {
            ...
            for (var i = 0; i < powerShellModules.length; i++) {
              ...
              var powerShellModuleFolderName = path.basename(
                powerShellModuleDirName
              );
              fs.copySync(
                powerShellModuleDirName,
                path.join(psModulesPath, powerShellModuleFolderName),
                { clobber: true, dereference: true }
              );
              console.log(
                `${powerShellModuleFolderName} copied in ps_modules for ${
                  project.name
                }`
              );
            }
